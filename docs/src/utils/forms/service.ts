import { CONTACT_TYPE_LABELS, HONEYPOT_FIELD } from './constants';
...
import { resolveContactRecipients, resolveSubscribeRecipients } from './routing';
import { createSessionCookie, issueFormToken, verifyIssuedToken } from './session';
...
import type { CloudflareRuntime, D1DatabaseLike, KvNamespaceLike } from '../../types/cloudflare';
...
async function sendContactEmail(
	...
	const recipients = resolveContactRecipients(payload.contactType);
...
async function handleSubscribe(
	...
	const notifyRecipients = resolveSubscribeRecipients();
	const ipHash = await hashValue(remoteIp || 'unknown');
...
export async function handleTokenRequest(context: APIContext) {
	...
	const issued = await issueFormToken(formType);
...
export async function handleFormSubmission(context: APIContext) {
	...
	const runtime = resolveFormRuntimeCapabilities(context.locals);
	...
	const cookieSessionId = context.cookies.get('karyon_form_session')?.value ?? '';
	const postedSessionId =
		typeof formData.get('formSession') === 'string' ? String(formData.get('formSession')).trim() : '';
	...
	let tokenResult:
		| Awaited<ReturnType<typeof verifyIssuedToken>>
	...
	if (!sessionCandidates.length) {
		...
		for (const sessionId of sessionCandidates) {
			const candidateResult = await verifyIssuedToken(formToken, sessionId, payload.formType);
	...
	try {
		if (payload.formType === 'contact') {
			await sendContactEmail(payload, remoteIp, context.request, runtime);
		} else {
			await handleSubscribe(payload, remoteIp, context.request, subscriberStore, runtime);
		}
	} catch (error) {
