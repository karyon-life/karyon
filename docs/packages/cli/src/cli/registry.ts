...
export const TRESEED_COMMAND_SPECS: TreeseedCommandSpec[] = [
	command({
		name: 'init',
		aliases: [],
		group: 'Workflow',
		...
		usage: 'treeseed init <directory> [--name <site-name>] [--slug <slug>] [--site-url <url>] [--contact-email <email>] [--repo <url>] [--discord <url>]',
		...
		examples: ['treeseed init docs-site --name "Docs Site" --site-url https://docs.example.com'],
	...
	command({
		...
		notes: ['Does not create branch preview deployments.'],
		related: ['deploy', 'destroy', 'start'],
		executionMode: 'handler',
		handlerName: 'config',
	}),
	command({
		...
		options: [{ name: 'preview', flags: '--preview', description: 'Create and publish a branch-scoped Cloudflare preview environment.', kind: 'boolean' }],
		examples: ['treeseed start feature/search-improvements', 'treeseed start feature/search-improvements --preview'],
		notes: ['Refuses to run with a dirty worktree or if the branch already exists.'],
		related: ['close', 'save', 'deploy'],
		executionMode: 'handler',
		handlerName: 'start',
	}),
	command({
		...
		notes: ['Only works on feature branches.', 'Refuses to run on staging or main.'],
		related: ['start', 'save', 'release'],
		executionMode: 'handler',
		handlerName: 'close',
	}),
	command({
		...
		notes: ['Persistent environments must already be initialized by treeseed config.', 'CI may infer staging from `staging` and production from `main`.'],
		related: ['config', 'start', 'destroy'],
		executionMode: 'handler',
		handlerName: 'deploy',
	}),
	command({
		...
		options: [{ name: 'hotfix', flags: '--hotfix', description: 'Allow save on main for an explicit hotfix.', kind: 'boolean' }],
		examples: ['treeseed save "feat: add search filters"', 'treeseed save --hotfix "fix: unblock production form submit"'],
		notes: ['Blocked on main unless --hotfix is provided.'],
		related: ['start', 'close', 'release'],
		executionMode: 'handler',
		handlerName: 'save',
	}),
	command({
	...
	command({
		...
		notes: ['Only for persistent environments. Branch cleanup belongs to treeseed close.', 'This command is destructive and requires explicit confirmation.'],
		related: ['config', 'deploy', 'close'],
		executionMode: 'handler',
		handlerName: 'destroy',
	}),
...
export const TRESEED_COMMAND_INDEX = new Map<string, TreeseedCommandSpec>();
