		...
		hooks: {
			'astro:config:setup'({ injectRoute }) {
				for (const route of PACKAGE_ROUTE_ENTRIES) {
					...
					if (route.pattern === '/api/form/submit' && tenantConfig.features?.forms === false) continue;
					injectRoute(route);
				}
			},
		},
	...
	return defineConfig({
		...
		env: {
			schema: {
				...
				DOCS_SMTP_USERNAME: envField.string({ context: 'server', access: 'secret', optional: true }),
				DOCS_SMTP_PASSWORD: envField.string({ context: 'server', access: 'secret', optional: true }),
				DOCS_SMTP_FROM: envField.string({ context: 'server', access: 'secret', optional: true }),
				DOCS_SMTP_REPLY_TO: envField.string({ context: 'server', access: 'secret', optional: true }),
				DOCS_FORM_TOKEN_SECRET: envField.string({ context: 'server', access: 'secret', optional: true }),
				DOCS_LOCAL_DEV_MODE: envField.enum({ values: ['cloudflare'], context: 'server', access: 'secret', optional: true }),
				DOCS_FORMS_LOCAL_BYPASS_TURNSTILE: envField.boolean({ context: 'server', access: 'secret', optional: true }),
				DOCS_FORMS_LOCAL_BYPASS_CLOUDFLARE_GUARDS: envField.boolean({ context: 'server', access: 'secret', optional: true }),
				DOCS_FORMS_LOCAL_USE_MAILPIT: envField.boolean({ context: 'server', access: 'secret', optional: true }),
				DOCS_MAILPIT_SMTP_HOST: envField.string({ context: 'server', access: 'secret', optional: true }),
				DOCS_MAILPIT_SMTP_PORT: envField.number({ context: 'server', access: 'secret', optional: true }),
			},
		},
		integrations: [
