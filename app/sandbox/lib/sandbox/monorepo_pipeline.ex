defmodule Sandbox.MonorepoPipeline do
  @moduledoc """
  Explicit engine-versus-target-workspace contract for the Chapter 11 monorepo pipeline.
  """

  @schema "karyon.monorepo-pipeline.v1"
  ...
  def validate_target_workspace(path, opts) when is_binary(path) and path != "" do
    ...
    cond do
      workspace == engine ->
        {:error, :engine_workspace_forbidden}

      within?(workspace, engine) ->
        {:error, :engine_workspace_forbidden}
